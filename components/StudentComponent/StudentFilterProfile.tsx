...
export default function StudentFilterProfile() {
  ...
  return (
    <div
      className={`font-Open-Sans ${openSans.className} flex items-center px-4 mt-[2rem] w-full justify-between`}
    >
      {/* filters container */}
      <div
        className={`flex gap-5 sm:w-full w-[20rem] ${
          isBelow1184 ? "flex-col items-start" : "items-end"
        } ${isBelow390 ? "w-[15rem]" : ""} ${isBelow310 ? "w-[13rem]" : ""}`}
      >
        {/* Class and Section Selection */}
        <header
          className={`flex flex-col gap-2 ${
            isBelow390 ? "w-[150px]" : "w-[200px]"
          }`}
        >
          ...
          <FormControl className="w-full">
            ...
            <button
              ...
              className="flex items-center w-full justify-between px-[11px] py-[15px] rounded-[13px] text-[var(--grey)] text-[18px] bg-[var(--secondary-text-color)] border-0 outline-none shadow-[0px_4px_4px_1px_rgba(138,135,135,0.3)]"
            >
              Select
              {/* {!selectOpen?<IoIosArrowDown className="text-[28px]" />:<IoIosArrowUp/>} */}
              {!selectOpen ? <ArrowDown01Icon /> : <ArrowUp01Icon />}
            </button>
            {/* -------- select dropdown -------- */}
            <div
              className={`flex flex-col ${
                selectOpen
                  ? "bg-[white] absolute top-[5rem] rounded-[9px] border border-solid border-[var(--border)] py-2 px-1 w-[13.5rem] shadow-[0px_4px_4px_1px_rgba(138,135,135,0.3)]"
                  : ""
              }`}
            >
              {selectOpen && (
                ...
                <div className="flex items-center relative">
                  ...
                  <button
                    ...
                    className="w-full max-w-[60%] gap-2.5 rounded-[3px] flex items-center justify-between border border-[var(--border)] p-2 text-[var(--primary-text-color)] z-0"
                  >
                    {selectedClass}
                    <BiSolidDownArrow className="text-[14px] fill-[var(--primary)]" />
                  </button>
